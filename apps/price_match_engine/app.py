...
# CONSERVATIVE: Only include highest-confidence conflicts to avoid false blocks
PRODUCT_LINE_CONFLICTS = [
    # JOTUN product lines - CRITICAL (paint brand conflicts are high-value)
    ...
    ('น้ำมันสน', 'ทินเนอร์'),
    ('TURPENTINE', 'THINNER'),
    # Product type conflicts (very different product types)
        ...
        for i, t in enumerate(target_products):
            ...
            if sim >= 18:  # Lower threshold for better recall
                ...
                if check_size_mismatch(source_name, t_name):
                    continue
                # PRE-FILTER: Skip candidates with door model mismatches
                if check_door_model_mismatch(source_name, t_name):
                    continue
...
Return: {{"match_index": <0-14 or null>, "confidence": <50-100>, "reason": "<brief reason for match>"}}
JSON only."""
    ...
    with tab1:
        ...
        with col1:
            ...
            if st.session_state.source_df is not None and len(st.session_state.source_df) > 0:
                st.dataframe(st.session_state.source_df, width="stretch", hide_index=True)
        ...
        with col2:
            ...
            if st.session_state.target_df is not None and len(st.session_state.target_df) > 0:
                st.dataframe(st.session_state.target_df, width="stretch", hide_index=True)
